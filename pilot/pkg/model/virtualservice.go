...
func stringMatchConflict(root, leaf *networking.StringMatch) bool {
	...
	if root.GetRegex() != "" || leaf.GetRegex() != "" {
		log.Warnf("It is a risk of conflict that either root(%s) or delegate(%s) have regex set, it is your own responsibility to prevent conflict.", root.GetRegex(), leaf.GetRegex())
	}
	// root is exact match
